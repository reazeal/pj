							...
							<div class="col-lg-12">
							 ...
							 <div class="col-lg-12" style="margin-top: 20px;width:100%;">
								 <table id="table"
								   ...
								   data-id-field="petugas_id"
								   data-page-list="[10, 25, 50, 100, ALL]"
								   data-show-footer="false"
								   data-side-pagination="server"
								   data-row-style="rowStyle"
								   data-url="<?php echo site_url('admin/petugas/get_data_petugas');?>"
								   >
								 </table>
							 </div>
...
<!-- Form Tambah Data -->
<div id="formTambahPetugas" class="modal fade" role="dialog">
        ...
        <div id="collapse2" class="body collapse in">
			<form class="form-horizontal" id="formTambahPetugasx">
				...
				<div class="form-group">
					...
					<div class="col-lg-6">
							<input class="validate[required] form-control"  type="text" name='kode' id='kode'  readonly="readonly"> 
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-lg-4">
						<div class="input-group input-append date" id="dp3" data-date="<?php echo date('d-m-Y'); ?>" data-date-format="dd-mm-yyyy">
							<input id="tgl_lahir" class="validate[required] form-control" value="<?php echo date('d-m-Y'); ?>" type="text" name='tgl_lahir'>
				...
				<div class="form-group">
					...
					<div class="col-lg-6">
							<input class="validate[required] form-control"  type="text" name='no_ktp' id='no_ktp'> 
					</div>
				</div>
...
<div id="formGantiFoto" class="modal fade" role="dialog">
  <div class="modal-dialog modal-md">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div id="collapse2" class="body collapse in">
		<form>
			<input class="form-control"  type="hidden" name='petugas_idx' id='petugas_idx' readonly=""> 
...
<script>
        ...
        $remove.click(function () {
          var $modal = $('#formTambahPetugas').modal({show: false});
			...
			if (confirm('Anda yakin untuk menghapus data ini ?')) {
				...
				$.ajax({ 
					type      : 'POST', 
					url       : '<?php echo site_url('admin/petugas/delete/');?>/',
	...
	function TambahData() {
		...
		$.ajax({
			...
			async: false,
			dataType: "html",
			url: "<?php echo site_url('admin/petugas/get_gen_kode');?>",
			success: function(data) {
				noPend=data;
			}
		});
		$modal.find('input[name="kode"]').val(noPend);
    ...
    function getIdSelections() {
        return $.map($table.bootstrapTable('getSelections'), function (row) {
            return row.petugas_id
        });
    }

    function responseHandler(res) {
        $.each(res.rows, function (i, row) {
            row.state = $.inArray(row.petugas_id, selections) !== -1;
        });
        return res;
    }
	...
	function showModal(title, row) {
        ...
        $modal.data('petugas_id', row.petugas_id);
    ...
    function operateFormatter(value, row, index) {
        return [
			'<a class="gantiFoto" href="javascript:void(0)" title="Ganti Foto">',
	...
	window.operateEvents = {
		'click .gantiFoto': function (e, value, row, index) {
			$modalFoto.data('petugas_id', row.petugas_id);
		    $modalFoto.find('input[name="petugas_idx"]').val(row.petugas_id);
        ...
        'click .remove': function (e, value, row, index) {
		    ...
		    if (confirm('Anda yakin untuk menghapus data ini ?')) {
                $.ajax({
                    url: '<?php echo site_url('admin/petugas/delete_id/');?>/',
					data :  { datanya : row.petugas_id },
                    type: 'POST',
                    success: function () {
                        $table.bootstrapTable('refresh');
                        MsgBox.show('Hapus Berhasil!');
	...
	$(document).ready(function() {
		...
		$("#fileuploader").uploadFile({
			...
			{
				//console.log($("#petugas_idx").val());
				var data ={ petugas_id: $("#petugas_idx").val()}
				return data;
			},
