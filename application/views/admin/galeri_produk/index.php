...
<div id="container" class="container" style="padding-top:60px; width: auto !important;">
	...
	<div id="body">
		...
		if($galeri_produk->num_rows() > 0) : ?>
			...
			<div align="center"><?= anchor('admin/galeri_produk/add','Tambah Produk',array('class'=>'btn btn-primary'))?></div>
			<hr />	
			<div class="row">
				<?php foreach($galeri_produk->result() as $img) : ?>
				<div class="col-md-3">
					<div class="thumbnail">
						<p>
						<?php 
						$data_gambar = explode(',',$img->filenya);
						foreach($data_gambar as $gambar) :
						  if($gambar!=''){
						  	echo " <img width='50' height='50' src=\"".site_url($gambar)."\" \">";
						  	echo "&nbsp;";	
						  }
						endforeach;
						...
						<i>
						<?php 
						$data_tags = explode(',',$img->nama_tagsnya);
						...
						<div class="caption">
							...
							<p><?php echo substr($img->description, 0,100)?>...</p>
							<p>
							
							<?php echo anchor('admin/galeri_produk/add_gambar/'.$img->id_galeri,'Pic',array('class'=>'btn btn-warning', 'role'=>'button')); ?>
							<?php echo anchor('admin/galeri_produk/add_tags/'.$img->id_galeri,'Tags',array('class'=>'btn btn-danger', 'role'=>'button')); ?>
							
								<?php echo anchor('admin/galeri_produk/edit/'.$img->id_galeri,'Edit',array('class'=>'btn btn-warning', 'role'=>'button')); ?>
								<?php echo anchor('admin/galeri_produk/delete/'.$img->id_galeri,'Del',array('class'=>'btn btn-danger', 'role'=>'button','onclick'=>'return confirm(\'Are you sure?\')'))?>
							</p>
						</div>
					</div>
				</div>
		...
		<?php else : ?>
			<?= anchor('admin/galeri_produk/add','Tambah Produk',array('class'=>'btn btn-warning', 'role'=>'button'))?>
		<?php 
