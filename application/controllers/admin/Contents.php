...
class Contents extends Admin_Controller
{
    ...
    public function create($content_type = 'page', $language_slug = NULL, $content_id = 0)
    {
        ...
        else
        {
            ...
            $share_fb = $this->input->post('share_fb');
            if ($content_id == 0)
            {
            ...
            $insert_translation = array('content_id'=>$content_id,'title' => $title, 'short_title' => $short_title, 'teaser' => $teaser,'content' => $content,'page_title' => $page_title, 'page_description' => $page_description,'page_keywords' => $page_keywords,'language_slug' => $language_slug, 'share_fb' => $share_fb);
    ...
    public function publish($content_id, $published)
    {
        ...
        if( ($content != FALSE) && ($published==1 || $published==0))
        {
            ...
            {
            	$datax = $this->content_translation_model->where(array('content_id'=>$content_id))->get();
            	if($datax->share_fb==1){
            		
            		$trans_id = $datax->id;
            		
            		$dt_slug  = $this->slug_model->where(array('translation_id'=>$trans_id))->get();
            		$url_slug = "Check my new post. ".site_url().$dt_slug->url; 
            		$message  = $datax->title.", ";
            		$message .= $datax->content;
            		
				    $yyy = $this->facebook->publish_text($url_slug." ".$message);	
				    if($yyy['code']=='463'){
