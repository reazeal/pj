...
class Languages extends Admin_Controller
{
	...
	public function edit_text($language_name = NULL)
	{
		$langnya = strtolower($language_name);

		if(!isset($langnya)){

		     $this->postal->add('File tidak diketemukan','error');
			 redirect('admin/languages');

		}else{
			$date_today = date('d_m_Y_h_i_s');
			$this->load->helper('file');
			$string = read_file('./application/language/'.$langnya.'/interface_lang.php');

			
			$arr_string = explode(';',$string);
			$this->data['file_bahasa'] = $string;
			
			$rules = array(
					'file_bahasa' => array('field'=>'file_bahasa','label'=>'File Bahasa','rules'=>'required')
				);
			...
			if($this->form_validation->run()===FALSE)
			{
				$this->render('admin/languages/edit_text');
				
			}else
			{
				
				write_file('./application/language/'.$langnya.'/interface_lang_backup_'.$date_today.'.php', $string); 
					 
				if ( ! write_file('./application/language/'.$langnya.'/interface_lang.php', $this->input->post('file_bahasa')))
				{
					 $this->postal->add('Tidak Bisa Ubah File','error');
				}
				else
				{
					 $this->postal->add('File Telah Diubah!','success');
				}
